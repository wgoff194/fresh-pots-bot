    ...
    def parse_event(self, event):
        if event and 'text' in event:
            ...
            if '!fp' in event['text'] or '!freshpots' in event['text']:
                ...
                string=request.split(event['text'].split()[1])[1].strip()

                print("Request: " + request + " | Command: " + command + " | String : " + string)
                self.handle_event(event['user'], command, event['channel'], request, string)
	
    def handle_event(self, user, command, channel, request, string):
        if command and channel:
            #print("Received command: " + command + " in channel: " + channel + " from user: " + user)
            response = self.command.handle_command(user,command,channel,request,string)
